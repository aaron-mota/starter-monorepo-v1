...
export function adapterFnDbToFE(data: TDocDb): TDoc {
  const { _id, createdAt, updatedAt, subscriptionUserId, familyId, ...rest } = data;
  return {
    id: adapterFnIdDbToFEClient(_id),
    createdAt: createdAt as Date,
    updatedAt: updatedAt as Date,
    subscriptionUserId: adapterFnIdDbToFEClient(subscriptionUserId),
    familyId: familyId ? adapterFnIdDbToFEClient(familyId) : undefined,
    ...rest,
  } as TDoc;
}
...
export function adapterFnFEToDbPartial(data: Partial<TDoc> | undefined): Partial<TDocDb> | undefined {
  ...
  const { id, subscriptionUserId, familyId, ...rest } = data;
  const dataDb: Partial<TDocDb> = {
    _id: id ? adapterFnIdFEToDbServer(id) : undefined,
    subscriptionUserId: adapterFnIdFEToDbServer(subscriptionUserId),
    familyId: familyId ? adapterFnIdFEToDbServer(familyId) : undefined,
    ...rest,
  };
...
export function adapterFnFEToDbCreate(data: TDocCreate): TDocDbCreate {
  const { subscriptionUserId, familyId, ...rest } = data;
  return {
    createdAt: new Date(),
    updatedAt: new Date(),
    subscriptionUserId: adapterFnIdFEToDbServer(subscriptionUserId),
...
export function adapterFnFEToDbUpdate(data: TDocUpdate): TDocDbUpdate {
  ...
  const dataDb: TDocDbUpdate = {
    updatedAt: new Date(),
    subscriptionUserId: subscriptionUserId ? adapterFnIdFEToDbServer(subscriptionUserId) : undefined,
    familyId: familyId ? adapterFnIdFEToDbServer(familyId) : undefined,
    ...rest,
  };
  Object.keys(dataDb).forEach(
